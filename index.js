...
function macSonucu(takimSkoruFunc, ceyrekSayisi) {
  let sayi1 = 0;
  let sayi2 = 0;
  for (let i = 1; i <= ceyrekSayisi; i++) {
    sayi1 += takimSkoruFunc();
    sayi2 += takimSkoruFunc();
  }
  return { evSahibi: sayi1, konukTakim: sayi2 };
}
...
function periyotSkoru(cb_takimSkoru) {
  return { "Ev Sahibi": cb_takimSkoru(), "Konuk Takim": cb_takimSkoru() };
}
